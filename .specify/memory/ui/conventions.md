...
- Collections (Fridge, History, Today log): empty vs data derived from array length; no remote loading state. Today / day-detail **data** is grouped into exclusive meal accordions (derived `Meal[]` from entry `ts`, consecutive-gap ≤ 30 min); expand index is session UI only (not persisted).
...
- `EntryRow` / `FoodRow` — list rows with overflow menu (FoodRow: Edit / Duplicate / Delete-custom; EntryRow: optional `readOnly` hides edit/delete)
- `MealList` / `MealAccordion` — exclusive meal accordions (Meal N + C/P/F/C header); shared by Today and DayDetailSheet
