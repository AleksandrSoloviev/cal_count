...
const SaveOneOffToFridgeSheet = ({ entry, onConfirm, onClose }: Props) => {
  const nameId = useId();
  const [name, setName] = useState(entry.foodName);
  const [nutrition, setNutrition] = useState<Nutrient>({ ...entry.nutrition });
    ...
    >
      ...
      <div className="relative bg-background rounded-t-3xl w-full max-w-md max-h-[90vh] overflow-y-auto pb-safe">
        ...
        <div className="px-5 pb-8">
          ...
          <p className="text-xs text-muted-foreground mb-6">{en.saveToFridge.hint}</p>
...
export default SaveOneOffToFridgeSheet;
