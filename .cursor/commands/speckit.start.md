...
4. Escalation trigger detection (for lite mode only):
   - Review the description for these triggers:
     ...
     8) meaningful UX change: the feature introduces new screens or changes the states/navigation of existing ones → recommend `--full`, and after `/speckit.plan` run `/speckit.uiplan`
        - **From `.specify/memory/ui/screens.md`** (read only if the description plausibly touches UI): consult the durable Screen Registry (`SCREEN-<slug>` rows) to judge this trigger against the actual current screen set — whether the work item adds a net-new screen or only changes existing ones — instead of grepping the codebase or guessing.
...
5. Generation behavior:
   - **From `.specify/memory/context.md`** (if present): load the project's `Current Scope` and `Non-Goals` before writing scope/constraints, so the generated artifact (lite `plan.md` or full `spec.md`) stays within the project's scope and does not propose out-of-scope work; if absent, derive scope from the work-item description and repo context.
...
6. Completion report:
   ...
   - In lite mode, suggest next steps: implement work → `finalize`. If the description seriously affects UI (new screens or states), note that `/speckit.uiplan` is available in lite too (it works from `plan.md` + the work-item description) and is recommended before implementing.
